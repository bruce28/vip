...
<tr><td>Name of Item Subcategory</td><td>Visible</td><td>Weight</td><td>Category</td><td>Cat. Num</td><td>Sub. Num</td><td>Add a new weight class</td><td>ATTS</td><td>ATTS</td></tr>
<?php
...
while($rek = mysql_fetch_array($result,MYSQL_BOTH)) { //changed from 1 
   ...
   echo '<tr style="bgcolor:whitesmoke;"><td>'.$rek["Name_sub"].'</td>'; 
	 echo '<td>'.manifest_visible($rek["kind"]).'</td>';
	...
	// echo '<td>'.$rek["Name_sub"].'</td>';
          ...
          // echo '<td>';
           ...
           echo "</form>";
           echo '<form action="categories_add.php" method="POST">';
           echo '<td>';
           $selected=manifest_dbi_get_att_name(manifest_dbi_get_att_sub_cat($id_c)); //selected one
           $id_c=$rek['id_c'];
           echo '<select name="atts">';
           for($z=0;$z<=manifest_dbi_get_number_elements_att();$z++)
           {
               if(manifest_dbi_get_att_sub_cat($id_c)==$z)
                    echo '<option value="'.$z.'" selected >'.manifest_dbi_get_att_name($z).'</option>';
               else   
           echo '<option value="'.$z.'" >'.manifest_dbi_get_att_name($z).'</option>';
           }
           echo '</select>';
           echo '</td>';
