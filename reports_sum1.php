...
function stock_fun()
{
$sql_select_site_dis="SELECT SUM(weight) as wei,item_has_cat.cat as cat_idd, SUM(cat) as quan FROM item_has_cat INNER JOIN Item ON Item.Item_has_cat_id_item_cat=item_has_cat.id_item_cat GROUP by cat_idd";
$result_site_dis=query_select($sql_select_site_dis);

//TESTED
$sql_select_site_dis_test="SELECT SUM(weight) as wei,item_has_cat.cat as cat_idd, SUM(cat) as quan FROM item_has_cat "
        . "INNER JOIN Item ON Item.Item_has_cat_id_item_cat=item_has_cat.id_item_cat INNER JOIN Barcode ON barcode.Item_id_item=item.id_item INNER JOIN Test ON Test.Barcode_id_Barcode=barcode.id_Barcode GROUP by cat_idd";
...
$sql_select_site_dis_test="SELECT SUM(weight) as wei,item_has_cat.cat as cat_idd, SUM(cat) as quan FROM item_has_cat "
        . "INNER JOIN Item ON Item.Item_has_cat_id_item_cat=item_has_cat.id_item_cat INNER JOIN Barcode ON barcode.Item_id_item=item.id_item INNER JOIN Test ON Test.Barcode_id_Barcode=barcode.id_Barcode Where Ready=0 GROUP by cat_idd";
$result_site_dis_test=query_select($sql_select_site_dis_test);
//shows tested all
...
echo '<tr><td colspan="1"></td><td><b>TOTAL WEIGHT Tested:</b> '.$total_sum_test.'<td><td colspan="5">'.$total.'</td></tr></br></table>';
...
echo '<tr><td colspan="1"></td><td><b>TOTAL WEIGHT STOCKED:</b> '.$total_sum.'<td><td colspan="5">'.$total.'</td></tr>';
echo '<tr><td colspan="1"></td><td><b>By this month:</b> '.$total_sum.'<td></tr>';
