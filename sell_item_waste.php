...
if($_GET['qc'] > 0)
{
    $qtty_bulk=$_GET['qc'];
    if($qtty_bulk==1)
     $message="Added ".$qtty_bulk." random item of ".get_item_name($_GET['item_type'])." category";         
    else
$message="Added ".$qtty_bulk." random items of ".get_item_name($_GET['item_type'])." category";    
}
...
<form action="sell_item_pro_waste.php" method="POST">
...
while($rek = mysql_fetch_array($result,1))  
  ...
  */echo "121";
  ...
  echo "</BR> </BR> <a style='text-decoration: none;
    ...
    border-collapse:collapse;  ' href='sell_item_waste.php?change_site=1'> Change Transaction </a> </BR>";
  }
  else
  {
...
<td>
    <select name="item_type" ><?php check_cat($_GET['item_type']); ?> </select>
</td>
...
<tr>
    <td></td><td><input type="text" name="quantity" value="" placeholder="Quantity"><td>
