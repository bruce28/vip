...
$sql='SELECT * FROM `Test`'
         ...
         . ' INNER JOIN Item ON Barcode.Item_id_item=Item.id_item WHERE Test.Ready=1';
...
<td>Reinstallation</td><td>Formatted</td>
<td>Ready</td><td>Date Tested</td></tr>
<?php
	  ...
	  echo '<td>'.test_string($rek["Functional_id_fun"]).'</td>';
		 echo '<td>'.test_string($rek["Cleaning_id_clean"]).'</td>';
          ...
          echo '<td>'.test_string($rek["Reinstallation_id_reinst"]).'</td>';
             ...
             echo '<td>'.$rek["meter"].'</td>';
              echo '<td>'.$rek["meter2"].'</td>';
               echo '<td>'.$rek["meter3"].'</td>';
			  echo '<td>'.$rek[12].'</td>';
	 echo '</tr>';
     //}
