...
if(isset($_POST['Finish'])) {
   ...
   $url="sell_item_invoice_waste.php?invoice=1&show=1&ref_sell=".$ref_sell;
...
if(isset($_POST['print_out']))
{
    ...
    //redirect to sell
    $url="sell_item_waste.php?print_out=1&item_type=".$_POST['item_type'];
    redirect($url,$TEST_T);
    exit();
}
...
while($rek = mysql_fetch_array($result,1))  
 ...
 //This here must be switched to 2 separate gueries to get pre sesion values or take from posted
  ...
  $na=$rek["id_Buyer"];
  $url='sell_item_waste.php?'.'site_specified='.$site_specified.'&'
...
{
    $item_type=$_POST['item_type'];
    $url='sell_item_waste.php?site_specified='.$site_specified.'&item_type='.$item_type;
    ...
    if(isset($_POST['quantity'])AND !empty($_POST['quantity']))
    {
        ...
        $insert_waste_qtty="INSERT INTO waste_quantity(qtty,name_cat,idtransaction_waste) VALUES('$qtty','$name_cat','$ref_sell_waste')"; 
...
if($v_bar_exist==1)
{
    ...
    $check_item_cat="SELECT * FROM item_has_cat WHERE id_item_cat='$item_type'";
    ...
    $insert_barcode="INSERT INTO waste_barcode(Barcode_waste,category,weight,type_item,stock_out) VALUES('$barcode2','$cat','$weight',$item_type,'1')";
    ...
    $url="sell_item_waste.php?item_type=$item_type&MESSG=2";
    redirect($url,$TEST_T);
    exit();
