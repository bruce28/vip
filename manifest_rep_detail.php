...
include 'functions_new_weights.php'; //ne calculation functions there

include 'functions_api.php'; //including new api
...
function get_weight_fully_reused($site_id)
{
    $sql="SELECT SUM(weight) as suma, site_id, Barcode.date,login,Ready,town,cat FROM barcode INNER JOIN test ON Barcode.id_Barcode=test.Barcode_id_Barcode
...
WHERE (Barcode LIKE '%DBS/%/1' OR Barcode LIKE '%DBS/%/2' OR Barcode LIKE '%UNQ/%') AND Site.site_id=$site_id";
...
echo "Item with Barcode <B>".$_GET['test']."</B> added to the System ";} ?>
    ...
    <form action="manifest_rep_detail.php" method="POST" >
 ...
 <?php
    ...
    $rek_list=mysql_fetch_array($result_list);
        ...
        //here in this module we come with all new weights dynamicly callculated thr new standard nonstandard modul, sum weight in sub cat ignored
        //this manifest_print_detail will be used as comparison to normal raport modul, for now
        ...
        $site_id=$rek_list['site_id'];
        $col=$rek_list['sum'];      
       //echo "<td> Status:<b> ". $re=round(get_weight_fully_reused($site_id),0)." kgs reused</b></td>";
       
        $factor=$re/$col;
        $factor=$factor*100;
        $factor=round($factor,1);
      // echo "<td>Factor: ".$factor."%</td>";
        echo "</tr>";
        echo "<tr></tr><tr></tr><tr></BR><td colspan='4'></td><td style='font-size:11px'>";
        //10 cat 1.
        echo "<form action='colre_report.php' method='POST'>";
...
//echo "ssd";
     ...
     //   echo "<input type='Submit' name='ten_cat' value='Generate 10 categories Sheet'>";
       ...
       //end 10 cat here second
        ...
        echo "<tr><td colspan='6'></td><td style='font-size:11px'>";
        //10 cat 1.
        echo "<BR/>";
        echo "<form action='colre_report.php' method='POST'>";
...
//echo "ssd";
     ...
     //   echo "<input type='Submit' name='ten_cat' value='Generate 10 categories Sheet'>";
        echo "<input type='Submit' name='tv_cat' value='Check Collection'>";
