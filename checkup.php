...
echo "<select name='id_check'><option value='1'>Everything</option><option value='2'>Failed test</option><option value='3'>Weights Tested</option><option value='4'>Users</option><option value='5'>Sold</option><option value='6'>Picked up</option><option value='7'>Cat</option></select>";
...
$site="SELECT SUM(weight), site_id, Barcode.date,login,Ready,town,item.name,cat FROM barcode INNER JOIN test ON Barcode.id_Barcode=test.Barcode_id_Barcode
INNER JOIN user_2 ON user_2.id_user=test.user_2_id_user 
INNER JOIN Item on Barcode.Item_id_item=Item.id_item
...
WHERE (Barcode LIKE '%DBS/%/1' OR Barcode LIKE '%UNQ/%') GROUP BY site_ref_number,name";

if($_POST['id_check']==6)
{
    echo "6";
$sql="
    SELECT SUM(weight), site_id, Barcode.date,login,Ready,town,item.name,cat FROM barcode INNER JOIN test ON Barcode.id_Barcode=test.Barcode_id_Barcode
INNER JOIN user_2 ON user_2.id_user=test.user_2_id_user 
INNER JOIN Item on Barcode.Item_id_item=Item.id_item
INNER JOIN item_has_cat ON Item.Item_has_Cat_id_item_cat=item_has_cat.id_item_cat
INNER JOIN site ON site.site_id=barcode.Site_site_id
...
if($_POST['id_check']==7)
{
    echo "7";
$sql="
    SELECT SUM(weight), site_id, Barcode.date,login,Ready,town,cat FROM barcode INNER JOIN test ON Barcode.id_Barcode=test.Barcode_id_Barcode
INNER JOIN user_2 ON user_2.id_user=test.user_2_id_user 
INNER JOIN Item on Barcode.Item_id_item=Item.id_item
INNER JOIN item_has_cat ON Item.Item_has_Cat_id_item_cat=item_has_cat.id_item_cat
INNER JOIN site ON site.site_id=barcode.Site_site_id
INNER JOIN Origin On Origin.Origin_id=site.Origin_origin_id
WHERE (Barcode LIKE '%DBS/%/1' OR Barcode LIKE '%UNQ/%') GROUP BY site_ref_number,cat
