...
{
    redirect("revision_stream.php",0);
...
<div id="tabel_wrap_out"></BR>
    ...
    <center><p style=""><h3>Streams Assignment and Calculation Module</h3></p></center>
    </br>
...
<tr><td>Category Description</td><td>Category</td><td>Stream Reference</td><td>Active Stream Subset</td><td>Manifest Stream</td></tr>
<?php
   ...
   //echo '<tr><td>'.$rek["Name_sub"].'</td>'; 
	// echo '<td>'.$rek["kind"].'</td>';
	 ...
	 echo '<td>'.$rek["name_cat"].'</td>';
	   echo '<td>'.$rek["id"].'.</td>';
            echo '<td> <form action="revision_stream.php" method="POST">';
...
echo '</td><td><input style="padding:10px;margin:10px" type="text" placeholder="Specify Stream.." name="batch_date" value="'.htmlentities($weight).'">';
...
echo "<input style='padding:10px;margin:10px' type='submit' name='Submit' value='Streams' align='right'>";
...
?>
 ...
 echo '<h3>Streams Range definition:</h3>';
...
echo "<center><form action='revision_stream.php' method='POST'>";



echo "Define a new Stream category";

echo '<input style="padding:10px;margin:10px" type="text" placeholder="Stream.." name="category" value="">';
echo 'with';
