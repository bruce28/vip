...
$v_test_bar_id=check_test($barcode_id); 
 ...
 {    $meter=2;
      ...
      $_SESSION['meter2']=$meter2;
      $_SESSION['meter3']=$meter3;
      $MESSG1=11;
...
)
{
  ...
  echo mysql_query($insert1) or die(mysql_error());
   ...
   else if($meter==2)  //meter specified
   {
       ...
       if($meter_out>0)
       $insert1 = "INSERT INTO `test` (`User_2_id_user`, `Barcode_id_Barcode`, `Pat_id_pat`, `Cleaning_id_clean`, `Functional_id_fun`, `Visual_id_visual`, `Ready`,`state`,Formatted_id_form,Reinstallation_id_reinst,date,Fuse_id_fuse,meter,meter2,meter3) VALUES ('$user','$barcode_id','$pat', '$cln', '$fun', '$vis', '$ts','$state1','$frm','$rin',CURDATE(),'$fuse','$meter_out','$meter2','$meter3')"; 
...
*/
 ...
 // echo mysql_query($update1) or die("Blad w zapytaniu!");
  ...
  $url="add_stock.php";
     //$url="add_barcode1.php?test=".$barcode."&MESSG=1";
   //concat($url,$check);
     redirect($url,$TEST_T);
     exit();
...
$url='add_barcode1.php?'.'bar_ex='.$v_bar_exist.'&'
.'v_pat='.$v_pat.'&'
.'v_fun='.$v_fun.'&'
.'v_cln='.$v_cln.'&'
.'v_vis='.$v_vis.'&'
