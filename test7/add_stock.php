...
echo "<BR/>EXECUTION TIME: ".$_GET['exec']." MS";
?>
...
<?php
//functionality receiving argument from stock processor
...
<tr>
<!--<td>Functionality</td><td>PAT test</td>-->

<?php
if(isset($_GET['barcode_container']))
        $barcode_container=$_GET['barcode_container'];

if(!empty($_GET['barcode_container'])AND !empty($_GET['V_BARCODE']))
    $autofocus_item=1;
?>
...
<td>
    <input type="text "id="code_read_box" name="barcode" value="<?php if(!empty($BAR_EX)) echo $BAR_EX; ?>" <?php if(empty($_GET['unq'])) echo "autofocus='' 'disabled'"; ?> placeholder="Barcode" <?php if($BLOCK_FORM==1) echo 'disabled'; ?> /><?php if(!empty($bar_ex_id)) echo "*"; ?>
</td>

<?php if(!empty($_GET['unq'])) echo '<input type="text "id="code_read_box" name="barcode2" '
    . 'value="'.$barcode_container.'" autofocus="" placeholder="Container Barcode" /><?php if(!empty($bar_ex_id)) echo "*"; ?>'; ?> 

<td>
<input type="text "id="" name="item" placeholder="e.g. DVD, PS2..." value="<?php if(!empty($ITEM_EX)) echo $ITEM_EX; ?>" <?php if($BLOCK_FORM==1) echo 'disabled';  if($autofocus_item==1) echo 'autofocus=""'; ?> /><?php if(empty($item_ex_id)) echo "+"; ?> 
</td>
<td>
...
<!--
<td>
<input type="radio" name="fun" value="1" <?php //if(isset($FUN_CHK1))echo $FUN_CHK1;if($BLOCK_FORM==1) echo 'disabled';?>  /> YES</br>
     <input type="radio" name="fun" value="0" <?php //if(isset($FUN_CHK2))echo $FUN_CHK2;if($BLOCK_FORM==1) echo 'disabled';?>  /> NO
</td>
<td>
</br>
<input type="radio" name="pat" value="1" <?php //if(isset($PAT_CHK1)) echo $PAT_CHK1;if($BLOCK_FORM==1) echo 'disabled'; ?> /> <B>CLASS 1</B> </BR>
<input type="radio" name="pat" value="0" <?php //if(isset($PAT_CHK2))echo $PAT_CHK2; if($BLOCK_FORM==1) echo 'disabled';?> /> <B>CLASS 2</B> </BR>
<input type="radio" name="pat" value="2" <?php //if(isset($PAT_CHK3))echo $PAT_CHK3; if($BLOCK_FORM==1) echo 'disabled'; ?> /> <B>Not Applicable</B>
</td>--> 
</BR></BR></BR>
<?php
...
<input type='submit' style="padding:9px;  
    ...
    border-collapse:collapse; " name='Submit' value='Add' align='right' <?php if($BLOCK_FORM==1) echo 'disabled'; ?>></br>
<input type='submit' style="padding:9px;  
    border: solid 1px #E5E5E5;  
    outline: 0;  
    font: normal 13px/100% Verdana, Tahoma, sans-serif;  
    ...
    border-collapse:collapse; " name='Cancel' value='Cancel' align='right'>

</td>

</tr>
<tr><td>
        <input type="checkbox" name="unq" value="1" <?php if(!empty($_GET['unq'])) echo "checked";  ?> disabled /> UNQ Mode
</td></tr>
</form>
...
<?php endif ?>    
 ...
 {
    echo "in test2;".$_SESSION['test_bar']; 
    if(isset($_GET['test']))
    // $_SESSION['test_bar']=$_GET['test']; 
        ;
    else
       ;
 }
 ...
 $_SESSION['test_bar']=$_GET['test'];    
     ...
     if($_GET['function']==1 OR $_POST['function']==1):   
 ?>
 ...
 <table  border="1"  style="padding:9px;  
    ...
    border-collapse:collapse;">
     ...
     <tr><td>Functionality</td><td>PAT test</td></tr> 
     <tr>
     <td>
     <input type="radio" name="fun" value="1" <?php if(isset($FUN_CHK1))echo $FUN_CHK1;if($BLOCK_FORM==1) echo 'disabled';?>  /> YES</br>
     <input type="radio" name="fun" value="0" <?php if(isset($FUN_CHK2))echo $FUN_CHK2;if($BLOCK_FORM==1) echo 'disabled';?>  /> NO
...
<td>
    <input type="hidden" name="test" value='<?php echo $_SESSION['test_bar'];?>'>
    ...
    border-collapse:collapse; " name="test2" value="test"> 
</td>
</tr>     
     </form>
 </table>
...
echo "<table width='85%'style='padding:9px;  
    border: solid 1px #E5E5E5;  
    outline: 0;  
    font: normal 13px/100% Verdana, Tahoma, sans-serif;  
    width: 50%;  
 ...
 echo $error_msg;   
   ...
   if(isset($_POST['fun'])AND $_POST['fun']==0)
     {
  ...
  $barcode_id=$rek['id_Barcode'];
         ...
         //check if id barcode already in test
          
         $sql = "SELECT * FROM Test Where Barcode_id_Barcode='$barcode_id'"; 
	
	echo "valid2"; 
  $result= mysql_query($sql) or die("SELECT BAR Wrong");
  $rek=mysql_fetch_array($result); 
        if($barcode_id==$rek['Barcode_id_Barcode'])
            echo "Barcode already tested";
        else
        {    
             
         $pat=$cln=$fun=$vis=$ts=$state1=$frm=$rin=$fuse=0;
         $user=$id_user;
         
         $insert1 = "INSERT INTO `test` (`User_2_id_user`, `Barcode_id_Barcode`, `Pat_id_pat`, `Cleaning_id_clean`, `Functional_id_fun`, `Visual_id_visual`, `Ready`,`state`,Formatted_id_form,Reinstallation_id_reinst,date,Fuse_id_fuse) "
                 . "VALUES ('$user','$barcode_id','$pat', '$cln', '$fun', '$vis', '$ts','$state1','$frm','$rin',CURDATE(),'$fuse')"; 
	 
	 
	 $barcode=swap_hash($barcode);
  echo$res= mysql_query($insert1) or die(mysql_error());
  if($res==1)
  {
  $url="add_stock.php?test=".$barcode.'&MESSG='.$MESSG;
   //concat($url,$check);
     redirect($url,1);
     exit();
  }
 ...
 if(isset($_POST['fun']) AND isset($_POST['pat']))
 {
 ...
 $V_BARCODE=$_SESSION['test_bar'];
         //$_GET['test'];
 $url="add_barcode1.php?test_stock=1&V_BARCODE=";
 $url.=$V_BARCODE;
 $url.="&patt=".$_POST['pat'];
 
 $_SESSION['patt_var']=$_POST['pat'];
