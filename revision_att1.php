...
while($rek = mysql_fetch_array($result,MYSQL_BOTH)) { //changed from 1 
   ...
   //echo '<tr><td>'.$rek["Name_sub"].'</td>'; 
	// echo '<td>'.$rek["kind"].'</td>';
	 ...
	 echo '<td>'.$rek["att"].'</td>';
	   echo '<td>'.$rek["cat_att"].'.</td>';
            echo '<td> <form action="revision_att1.php" method="POST">';
...
{  
    ...
    $sql_check_trans="SELECT * FROM att WHERE cat_att='$att_trans'";
...
if(isset($_POST['category']))
{
    ...
    $insert_sql_att="INSERT INTO att(att, cat_att) VALUES('$att_key','$category')";
...
?>
 ...
 $sql_atts_range="SELECT * FROM att";
...
echo "<center><form action='revision_att1.php' method='POST'>";
