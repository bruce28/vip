...
<tr><td>Barcode</td><td>Item</td><td>Brand</td><td>Serial</td><td>Visual</td><td>PAT </td><td>Functionality</td><td>Cleaning</td><td>Formatted</td><td>Reinstallation</td><td>Correct Fuse Fitted</td><td>Repairs carried out</td></tr>
<tr>


<?php if(!empty($item_ex_id)AND !empty($item_ex_id)) $COL=1; else $COL=0?>

<form onsubmit="return ini()" action="add_process_bar.php" method="POST">
...
<?php if($_SESSION['meter']==2 OR $_GET['meter_out']>0):?>
<tr>
    <td colspan="4"></td><td  style="background-color: whitesmoke" ></td><td><input  type="text" placeholder="Meter Output" value="<?php if(isset($_GET['meter_out'])) echo $_GET['meter_out']; ?>" name="meter" style="padding:9px;  
    border: solid 1px #E5E5E5;  
    outline: 0;  
    font: normal 13px/100% Verdana, Tahoma, sans-serif;  
    width: 100%;  
...
<span id="buttons_out"> 
 ...
 <p class="submit">  
      <a href="index.php"> <button style="padding:9px;   
    border: solid 1px #E5E5E5;  
    outline: 0;  
    font: normal 13px/100% Verdana, Tahoma, sans-serif;  
    width: 10%;  
