...
function num_select()
{
    $sql="SELECT DISTINCT priv FROM User_2";
...
<div id="tabel_wrap_out"></BR>
    ...
    <center><p style=""><h3>User Management and Logs Module</h3></p></center>
    </br>
...
if(isset($_POST['category']))
{
    ...
    $priv=$_POST['priv'];
    $insert_sql_att="INSERT INTO User_2(login, pass, name, priv) VALUES('$category','$att_key','$user_name','$priv')";
...
?>
 ...
 echo '<h3>User definition:</h3>';
 
 $sql_atts_range="SELECT * FROM User_2";
...
echo "<center><form action='revision_user.php' method='POST'>";



echo "Add a System User";

echo '<input style="padding:10px;margin:10px" type="text" placeholder="Login.." name="category" value="">';
echo ' with password';
echo '<input style="padding:10px;margin:10px" type="text" placeholder="Password.." name="att_key" value="">';
echo '<input style="padding:10px;margin:10px" type="text" placeholder="Name.." name="user_name" value="">';
...
for($i=0;$i<num_select();$i++)
{
  ...
  echo '<option value="'.$i.'">'.priv($i).'</option>';  
...
echo "<input style='padding:10px;margin:10px' type='submit' name='Submit' value='Create' align='right'>";
...
?>   
 ...
 echo '<h3>User Logs and Activity:</h3>';
 
 $sql_atts_range="SELECT * FROM User_2";
 $query_range=array();
 $preset_name=array();
 $preset_name[0]="Parceling Summary:";
 $preset_name[1]="Stocking Summary:";
 $preset_name[2]="Testing Summary:";
  $preset_name[3]="Manifest Summary:";

$query_range[0]="Select Count(*) as count FROM six_barcode WHERE parceler='";  
$query_range[1]="Select Count(*) as count FROM barcode WHERE user_2='";  
$query_range[2]="Select Count(*) as count FROM test WHERE User_2_id_user='";
$query_range[3]="Select DISTINCT Count(*) as count,Site_site_id FROM delivery WHERE picker1='";
...
//$preset_name[3]=":";
 ...
 while($rek_att=  mysql_fetch_array($result_att))
 {
     $tmp_query=$query_range[$i];
     echo '</b><td>';
     echo $rek_att[1]." ";
     $tmp_query.=$rek_att[0]."'";
     $result=mysql_query($tmp_query) or die(mysql_error());
